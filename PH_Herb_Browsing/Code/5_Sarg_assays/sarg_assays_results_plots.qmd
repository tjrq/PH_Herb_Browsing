...
load("Model_Outputs/logm_sarg96h50m_stand_mod.Rdata")
```
...
```{r}
logm_sarg96h50m_stand_mod |> as_draws_df() |> 
  dplyr::select(matches("^b_.*")) |> 
  exp() |> 
  summarise_draws(median,
                  HDInterval::hdi,
...
```{r}
sarg_assay_96h50m_stand_nd <-
  logm_sarg96h50m_stand_mod |> 
  emmeans(~ Depth_band *
            Treatment, 
          type = "link") |> 
  gather_emmeans_draws() |> 
